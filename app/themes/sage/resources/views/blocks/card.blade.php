{{--
  ...
  @endphp
  <div class="section-card--builder flex {{ $card_ord }} md:flex-no-wrap md:items-center md:justify-between w-full brm-card">
    <div class="card-content w-full md:w-1/2">
      <div class="content--inner">
        {!! $card_cnt !!}
      </div>
    </div>
    <div class="w-full md:w-1/2 py-32 mt-8 md:mt-0 md:py-64 bg-center bg-no-repeat bg-cover card-image js-card {{ $background_state }}" style="background-image: url({{ $card_dsk }})" data-mobile="{{ $card_mbl }}" data-desktop="{{ $card_dsk }}"></div>
  </div>
  @endif
