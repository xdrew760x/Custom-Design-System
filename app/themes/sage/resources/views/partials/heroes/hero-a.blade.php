...
<section id="{{ $block['keywords'][0] }}" class="w-full brm-hero" role="region" aria-label="Hero">
  <div class="section-brm--hero flex flex-col flex-wrap justify-center items-center bg-no-repeat js-background" style="background-image:url({{ $options['desktop'] }}); min-height: {!! $max_height !!}px;" data-mobile="{{ $options['mobile'] }}" data-desktop="{{ $options['desktop'] }}">
