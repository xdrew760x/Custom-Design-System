...
<section class="section section--{{ $i }} section--{{ $section_state }} {{ $order_state }} {{ $background_color_state }}">
  <div class="container flex justify-between">
    ...
    <div class="section__bg {{ $background_state }} w-full md:w-1/2 bg-contain bg-no-repeat bg-center" style="background-image: url({{ $bg_mobile }})" data-mobile="{{ $bg_mobile }}" data-desktop="{{ $bg_desktop }}"></div>
    @if( $content )
