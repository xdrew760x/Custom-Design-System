...
<div class="footer-b">
  ...
  <div class="footer__column footer__top md:flex md:flex-row md:items-start md:justify-between">
    ...
    <div class="column">
      ...
      <p>{!! get_bloginfo() !!}<br>
        {!! $address_one !!}<br>
        {!! $address_city !!}, {!! $address_state !!} {!! $address_zipcode !!}</p>
        @endif

        @if ($address_fax_number)
        <a href="tel:{{ preg_replace('/[^0-9]/', '', $address_fax_number) }}" class="desk-none">{!! $address_fax_number !!}</a>
        @endif

        @if ($address_email)
        <p><a href="mailto:{{ $address_email }}">{!! $address_email !!}</a></p>
        @endif
